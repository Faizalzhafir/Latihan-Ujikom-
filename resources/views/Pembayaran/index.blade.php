        ...
        <div class="col-12">
                <div class="card mb-0">
                    ...
                    <div class="d-md-flex justify-content-between mb-9">
                        ...
                        <p class="card-subtitle mb-0">Lihat Data Pembayaran </p>
                        </div>
                    ...
                    <div class="table-responsive overflow-x-auto latest-reviews-table">
                        ...
                        <tbody>
                            ...
                            <td>
                                <p class="mb-0">{{ $item->nama_pembayaran }}</p>
                            </td>
                            <!-- ======= CRUD Method ======= -->
                            <td>
                                ...
                                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                    <li>
                                    <a class="dropdown-item d-flex align-items-center gap-3" href="{{ route('pembayaran.create') }}">
                                        <i class="fs-4 ti ti-plus"></i>Add
                                    </a>
                                    </li>
                                    <li>
                                    <a class="dropdown-item d-flex align-items-center gap-3" href="{{ route('pembayaran.edit', ['id' => $item->id_pembayaran ]) }}">
                                        <i class="fs-4 ti ti-edit"></i>Edit
                                    </a>
                                    </li>
                                    <li>
                                    <form action="{{ route('pembayaran.delete', ['id' => $item->id_pembayaran ]) }}" method="POST" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
...
<body>
    <h1 align="center">Jenis Pembayaran</h1>
    <h3 align="center">Data Pembayaran</h3>
    <a href="{{ route('pembayaran.create') }}">Tambah</a>
        ...
        <tbody>
            @foreach ($pembayaran as $item)
                <tr>
                    ...
                    <td>
                        <a href="{{ route('pembayaran.edit', ['id' => $item->id_pembayaran] ) }}">Edit</a>
                        <form action="{{ route('pembayaran.delete', ['id' => $item->id_pembayaran]) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" onclick="return confirm('Apakah kamu yakin akan menghapus {{ $item->nama_pembayaran }} ini?')">Hapus</button>
                        </form>
                    </td>
                </tr>
            @endforeach
